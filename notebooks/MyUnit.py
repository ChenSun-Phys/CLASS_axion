...
This module defines a classes called SIUnit and NaturalUnit. It is based on class Unit and utilizes Sympy to parse 
...
class SIUnit(Unit):
    def __init__(self,input):
        ...
        eps0_val = 8.854187817620*10**(-12)*A**2*s**4/kg/m**3 # A**2 s**4 kg**-1 m**-3
        e_val = 1.6021766208*10**(-19)*A*s # C
        T_val = kg/A/s**2 
        G_val = 10**(-4)*kg/A/s**2
        
        list_symbol = np.array(symbols("h, hbar, eV, c, kB, Alfa, GN, mu0, eps0, e, T, G"))
        list_val = np.array([h_val, hbar_val, eV_val, c_val, kB_val, Alfa_val, GN_val, mu0_val, eps0_val, e_val, T_val, G_val])
    ...
    def __init__(self,input):
        ...
        GN_val = (6.67408*10**-11)*\
          ((1.602176565*10**-19)/(1.054571800*10**-34)/(299792458))**3/\
            ((299792458)**2/(1.602176565*10**-19))/((1.602176565*10**-19)/(1.054571800*10**-34))**2*eV**-2
        ...
        Om_b_val = 0.04842 
        Om_c_val = 0.2580 
        Om_g_val = 0.0000538357 # aquired by integrating 2.726K black body  c.f. Hubble_BAO.nb for example
        Om_nu_val = 3.4*10**-5 
        h0_val = 0.678
        H0_val = 67.8*10**3*m_val/s_val/(3.0857e22 * m_val) # arXiv:1502.01589 
        rho_c_val = 3.721623621707084e-11  * (eV**4)# 3H0**2/(8\[Pi]*GN)/.physQuantity/.physSI2Natural*)
        
        
        # first round of sub
        list_symbol = np.array(symbols("h, hbar, c, kB, mu0, eps0, J, e, Alfa,\
                            T, kg, s, m, cm, GN, AlphaMZ, GF, K, A, fm, nm,\
                            cm, km, au, ly, pc, kpc, Mpc, Gpc, year,\
                            keV, MeV, GeV, TeV"))
        list_val = np.array([h_val, hbar_val, c_val, kB_val, mu0_val, eps0_val, J_val, e_val, Alfa_val,\
                             T_val, kg_val, s_val, m_val, cm_val, GN_val, AlphaMZ_val, GF_val, K_val, A_val, fm_val, nm_val,\
