...
int background_functions(
  ...
  //CS
  double scf_rho, scf_p; //, scf_rho_prime, scf_p_prime;
  double scf_w; 
  /* used when we match the scf to fld */
  //SC
  double dp_dloga;

  //CS
  /* define quantities to determine when the scalar starts to oscillate*/
  double scf_m;
  double scf_a_crit, scf_tau_crit, scf_k_crit, scf_end_of_scf;
  //int scf_flg_just_flipped = _FALSE_; 
  //SC
  ...
  if (pba->has_scf == _TRUE_) {
    ...
    if (pba->scf_rec_approx_flg == _TRUE_){
      /* this corresponds to the edge of the intervals, tau_start */
      if (a < scf_a_crit * scf_end_of_scf) {
	pba->scf_fld_approx = _FALSE_; 
      }
      else {
    ...
    /* do the real work according to the  switch status */
    if (pba->scf_fld_approx == _FALSE_) {
	...
	pvecback[pba->index_bg_phi_scf] = scf_rho; /* note this is not the quantity being integrated, bg not bi
						    * need to be casted to pvecback_B later after calling _functions() */
	pvecback[pba->index_bg_phi_prime_scf] = phi_prime;   
	pvecback[pba->index_bg_V_scf] = V_scf(pba,phi); //V_scf(pba,phi); 
	pvecback[pba->index_bg_dV_scf] = 0.; // not used
	pvecback[pba->index_bg_ddV_scf] = 0.; //not used
	pvecback[pba->index_bg_rho_scf] = scf_rho ; /* the last time computing rho using phi */
	pvecback[pba->index_bg_p_scf] = scf_p;  /* the last time computing P using phi */
	rho_tot += pvecback[pba->index_bg_rho_scf];
	p_tot += pvecback[pba->index_bg_p_scf];
	dp_dloga += -3.*(scf_rho + scf_p)* scf_w; /* add it now */
	//divide relativistic & nonrelativistic (not very meaningful for oscillatory models)
	rho_r += 3.*pvecback[pba->index_bg_p_scf]; //field pressure contributes radiation
	rho_m += pvecback[pba->index_bg_rho_scf] - 3.* pvecback[pba->index_bg_p_scf]; //the rest contributes matter
	...
	scf_p = scf_w * scf_rho; 
	//scf_p_prime = (1. + scf_w)*scf_rho_prime;
	pvecback[pba->index_bg_phi_scf] = scf_rho; /* notice this is not the index_bi_index_scf. we set it to rho for consistency.*/
	pvecback[pba->index_bg_phi_prime_scf] = sqrt(3.*(scf_rho+scf_p)*a*a); 
	pvecback[pba->index_bg_V_scf] = 3./2. * (scf_rho - scf_p); //V_scf(pba,phi); //write here potential as function of P and rho
	pvecback[pba->index_bg_dV_scf] = 0.; // not used
	pvecback[pba->index_bg_ddV_scf] = 0.; //not used
	pvecback[pba->index_bg_rho_scf] = scf_rho;
	pvecback[pba->index_bg_p_scf] = scf_p;
	rho_tot += pvecback[pba->index_bg_rho_scf];
	p_tot += pvecback[pba->index_bg_p_scf];
	dp_dloga += -3.*(scf_rho + scf_p)* scf_w; /** <-- This is added now, using the continuity eqn*/
	//divide relativistic & nonrelativistic (not very meaningful for oscillatory models)
	rho_r += 3.*pvecback[pba->index_bg_p_scf]; //field pressure contributes radiation
	rho_m += pvecback[pba->index_bg_rho_scf] - 3.* pvecback[pba->index_bg_p_scf]; //the rest contributes matter
	//printf(" a= %e, Omega_scf = %f, \n ",a_rel, pvecback[pba->index_bg_rho_scf]/rho_tot );
  ...
  if (pba->has_scf == _TRUE_){
	...
	(-pvecback[pba->index_bg_phi_prime_scf]*pvecback[pba->index_bg_H]/a-2./3.*pvecback[pba->index_bg_dV_scf]);
      ...
      //printf("----switch to fld approximation\n");
      //printf("----a=%g, scf_a_crit=%g, scf_end_of_scf=%g\n", a, scf_a_crit, scf_end_of_scf);      
      pvecback[pba->index_bg_p_tot_prime] += 0.; // as it was added already after it switches to fluid picture
    }
    //SC end logic
  }
...
int background_indices(
  ...
  //CS
  //class_define_index(pba->index_bi_phi_switch_scf,pba->has_scf,index_bg,1);
  //SC
...
int background_solve(
  ...
  //CS set the first step to start with field picture
  if (pba->has_scf == _TRUE_){
    pba->scf_fld_approx = _FALSE_;      
  }
  //SC
  ...
  while (pvecback_integration[pba->index_bi_a] < pba->a_today) {
    ...
    /* CS turn on to allow background_functions to write to pvecback[pba->index_bi_phi_switch_scf] */
    /* this makes sure the appx is only turned on at the edge of the intervals to avoid messing with _derivs*/
    /* do we need to do the matching here if just flipped? */
    if (pba->has_scf == _TRUE_){
      pba->scf_rec_approx_flg = _TRUE_;
    ...
    if (pba->has_scf == _TRUE_){
      //CS turn off pba->scf_rec_approx_flg so when it's called by derivs it won't confuse derivs. I call it ``reading mode'' 
      pba->scf_rec_approx_flg = _FALSE_;
    ...
    /* -> perform one step */
    //CSSC: the code of generic_integrator shows it evolves (modifies) pvecback_integration from tau_start to tau_end
  ...
  //CS in the final looping go back to write mode to reproduce the above pvecback
  if (pba->has_scf) {
    pba->scf_fld_approx = _FALSE_;
    pba->scf_rec_approx_flg = _TRUE_;
  }
  ...
  for (i=0; i < pba->bt_size; i++) {
    ...
    //CS
    /* need to be careful here so that early phi_scf is not erased. */
    // since we turned off background_functions before derivs it should be fine.
    
    //printf("final loop: i'm about to call b_func(), with %s mode.\n", (pba->scf_rec_approx_flg)? "Writing": "Reading");
...
int background_derivs(
  ...
  if (pba->has_scf == _TRUE_){
    /** - before the switch scalar field equation: \f$ \phi'' + 2 a H \phi' + a^2 dV = 0 \f$  (note H is wrt cosmic time) */
    //if (pvecback[pba->index_bi_phi_switch_scf] == (double)_FALSE_) {
    if (pba->scf_fld_approx == _FALSE_) {
      ...
      /* dy[pba->index_bi_phi_prime_scf] = 0. ; //check if this makes phi still */
    }
    else {
      /** - after the switch, treat phi as a fluid */
      dy[pba->index_bi_phi_scf] = -3.*(pvecback[pba->index_bg_rho_scf] + pvecback[pba->index_bg_p_scf])*y[pba->index_bi_a]*pvecback[pba->index_bg_H];
      //dy[pba->index_bi_phi_scf] = 0. ; 
      dy[pba->index_bi_phi_prime_scf] = 0.;
      //dy[pba->index_bi_phi_switch_scf] = 0.;       
    }
  }
...
//CS
/**
 * Axion field potential and its derivatives with respect to the field _scf
 * - \f$ V = m^2 \phi^2/2 - m^2 \phi^2/f^2/24 \f$
 *
 * TODO:
 * - Add cosine potential 
 *
 */
 ...
 * - f is given in units of the reduced Planck mass \f$ m_{pl} = (8 \pi G)^{(-1/2)}\f$
